...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="生成代码质量 HTML 报告")
    default_root = Path(__file__).resolve().parent.parent
    parser.add_argument("--root", type=Path, default=default_root, help="项目根目录")
    parser.add_argument(
        "--output",
        type=Path,
        default=default_root / "tmp" / "code_quality_report.html",
        help="HTML 报告输出路径",
    )
    parser.add_argument(
        "--include-tests",
...
def main() -> None:
    args = parse_args()
    report = build_report(args.root, include_tests=args.include_tests)
    output_path = write_report(report, args.output)
    print(f"代码质量报告已生成: {output_path}")
    print(
