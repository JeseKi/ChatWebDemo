...
def test_email_change_code_sends_for_existing_email_but_confirm_rejects(test_client):
    first_email = "email-change-owner@example.com"
    resp = test_client.post(
        "/api/auth/send-verification-code", json={"email": first_email}
    )
    assert resp.status_code == 200, resp.text
    first_code = service.verification_codes[first_email]["code"]
    test_client.post(
        "/api/auth/register-with-code",
        json={
            "username": "email_change_owner",
            "email": first_email,
            "password": "Password123",
            "code": first_code,
        },
    ...
    second_email = "email-change-target@example.com"
    resp = test_client.post(
        "/api/auth/send-verification-code", json={"email": second_email}
    )
    assert resp.status_code == 200, resp.text
    second_code = service.verification_codes[second_email]["code"]
    test_client.post(
        "/api/auth/register-with-code",
        json={
            "username": "email_change_target",
            "email": second_email,
            "password": "Password123",
            "code": second_code,
        },
    ...
    login = test_client.post(
        "/api/auth/login",
        json={"username": "email_change_owner", "password": "Password123"},
    ).json()
    headers = {"Authorization": f"Bearer {login['access_token']}"}
    ...
    assert send_resp.json()["message"] == "验证码已发送"
    change_code = service.verification_codes[second_email]["code"]
    invalid_code = "000000" if change_code != "000000" else "111111"

    invalid_confirm_resp = test_client.post(
        "/api/auth/profile/email-change/confirm",
        json={"email": second_email, "code": invalid_code},
        headers=headers,
    )
    assert invalid_confirm_resp.status_code == 400
    assert invalid_confirm_resp.json()["detail"] == "验证码无效或已过期"

    confirm_resp = test_client.post(
        "/api/auth/profile/email-change/confirm",
        json={"email": second_email, "code": change_code},
        headers=headers,
    )
    assert confirm_resp.status_code == 400
    assert confirm_resp.json()["detail"] == "邮箱已被使用"
