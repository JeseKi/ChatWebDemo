...
def test_bootstrap_default_admin_allows_short_env_password(
    test_db_session: Session, monkeypatch
):
    """测试环境变量引导管理员不受接口密码长度校验限制"""
    monkeypatch.setattr(auth_config, "init_admin_name", "env_admin")
    monkeypatch.setattr(auth_config, "init_admin_email", "env-admin@example.com")
    monkeypatch.setattr(auth_config, "init_admin_password", "fsm123")

    bootstrap_default_admin(test_db_session)

    user = get_user_by_username(test_db_session, "env_admin")
    assert user is not None
    assert user.role == UserRole.ADMIN
    assert user.check_password("fsm123") is True
